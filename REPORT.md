...
The agent-facing layer is deliberately thin. FastAPI projects a catalog directly from artifacts and invokes the same replay engine as the CLI. API contracts, catalog projection, chat parsing, evidence projection, and live-run lifecycle are separate modules; `app.py` composes routes and execution. The dashboard ships as separately packaged HTML, CSS, and JavaScript rather than one embedded page. Synchronous endpoints preserve the simple agent contract, while watchable endpoints return a run ID immediately so the dashboard can poll redacted step-start/step-complete events. Invocation is serialized because this demo has one browser. A production version would replace that lock with a browser/session pool keyed by institution and operator session.

Quality gates are executable rather than aspirational: CI runs Ruff lint and formatting checks, mypy over the deterministic production core, an 80% branch-coverage floor, a hashed Python 3.11 dependency lock, and dependency vulnerability auditing. The same offline checks are available through `scripts/verify_submission.sh`.
...
The control state machine distinguishes `AUTOMATION_RUNNING`, `PAUSED`, `HUMAN_CONTROL`, `ABORTED`, and `DONE`, with an explicit owner and recorded transition actor. A terminal outcome with `recovery=escalate`, a policy refusal, exhausted recovery, or a discovery dead end creates an intervention containing capability/goal context, current step, reason code, live URL, screenshot reference, and timestamp. Automation releases ownership before waiting.

The CLI operator console and the dashboard API retain the same active surface and `SessionControl` while replay awaits a person. Serializable watchable-run state is atomically persisted beside run evidence, so a restart can still explain the last known state. The non-serializable browser handle and control token remain process-local while active. Taking control changes the owner; policy-checked actions are appended to the escalation; resume records retry, continue, or abort and hands the existing page back to replay. The dashboard displays current pixels from that shared session and provides these controls directly in Live processing. The included local harness demonstrates pause, scripted operator action, and completion on the same session. The live MERIDIAN teller-hold evidence demonstrates detection and context capture through the artifact/API path.

The dashboard does not bypass this model. Human actions are limited by the effective deployment/artifact host and action policy, and typed values are redacted from the handoff audit. High-risk calls require explicit confirmation, and an authorization failure remains typed even when invoked through chat or HTTP. Synchronous API calls return escalations immediately; watchable `/start` calls retain the shared session for the configured handoff window.
...
I kept one browser and filesystem evidence rather than building a queue, database, distributed scheduler, or browser fleet. Persisted live-state metadata improves restart diagnostics, but an active CDP session is deliberately leased to one process; horizontal execution would require an external worker lease and browser pool. The chatbot uses a transparent grammar instead of another model because capability selection was the point, not conversational coverage. The takeover panel is intentionally a minimal screenshot plus policy-checked action form, not real-time co-browsing. Desktop and screenshot adapters remain designed seams. There is no automatic compensating transaction for a successful write, and the demo does not reset the remote app.
