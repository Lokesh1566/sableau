...
class AnthropicPlanner:
    ...
    async def decide(self, goal, observation, history, context) -> Planned:
        ...
        return Planned(
            tool="finish",
            args={"status": "give_up", "summary": rationale or "no tool call"},
            rationale=rationale,
        )
...
class HeuristicPlanner:
    ...
    async def decide(self, goal, observation: Observation, history, context) -> Planned:
        ...
        if not has("type:search"):
            return Planned(
                "act",
                {
                    ...
                    "target_role": "textbox",
                    "target_label": "Claim, member or provider",
                },
        ...
        if "no claims match" in observation.text.lower():
            return Planned(
                "finish", {"status": "give_up", "summary": "the search returned no matching claim"}
            )
        ...
        if f"/claims/{claim_id}" in url and not url.endswith("receipt"):
            ...
            return Planned(
                "finish",
                {"status": "give_up", "summary": "saved the decision but no receipt appeared"},
            )
    ...
    def _decide_meridian(self, observation: Observation, history, context) -> Planned:
        ...
        def assertion(
            key: str, description: str, value: str, kind: str = "text_present"
        ) -> Planned:
            return Planned(
                "assert_state",
                {
        ...
        if (
            capability == "meridian_core.update_member_information"
            and "MEMBER INFORMATION UPDATED" in observation.text
        ):
            if not done("assert:member_information_updated"):
                return assertion(
                    "member_information_updated",
                    "The console confirms the member update",
                    "MEMBER INFORMATION UPDATED",
                )
            if not done("read:confirmation_message"):
                return act(
                    "Read the member update confirmation",
                    "read",
                    output="confirmation_message",
                    target_text="MEMBER INFORMATION UPDATED",
                )
            return Planned(
                "finish", {"status": "success", "summary": "Updated member contact information"}
            )
        ...
        if "/signon" in url:
            if current("operator") != str(params.get("operator", "")):
                return act(
                    "Enter the supplied Operator ID",
                    "type",
                    text=str(params.get("operator", "")),
                    target_name_attr="operator",
                )
            if current("password") != "[set]":
                return act(
                    "Enter the supplied password",
                    "type",
                    text=str(params.get("password", "")),
                    target_name_attr="password",
                )
            if current("branch") != str(params.get("branch", "")):
                return act(
                    "Select the supplied branch",
                    "select",
                    value=str(params.get("branch", "")),
                    target_name_attr="branch",
                )
            return act(
                "Submit the sign-on form", "click", target_role="button", target_name="Sign On"
            )

        if url.rstrip("/").endswith("/menu"):
            if not done("assert:main_menu_loaded"):
                return assertion(
                    "main_menu_loaded", "An authenticated MAIN MENU is displayed", "MAIN MENU"
                )
            if capability == "meridian_core.sign_on":
                if not done("read:session_status"):
                    return act(
                        "Read the authenticated session status",
                        "read",
                        output="session_status",
                        target_text="MAIN MENU",
                    )
                return Planned(
                    "finish", {"status": "success", "summary": "Signed on and verified MAIN MENU"}
                )
            return act(
                "Open Member Inquiry / Selection",
                "click",
                target_role="link",
                target_name="Member Inquiry / Selection",
            )
        ...
        if url.rstrip("/").endswith("/members") and "?" not in url:
            if not done("assert:member_inquiry_loaded"):
                return assertion(
                    "member_inquiry_loaded",
                    "Member Inquiry / Selection is displayed",
            ...
            if current("q") != query:
                return act(
                    "Enter the supplied member search value",
                    "type",
                    text=query,
                    target_name_attr="q",
                )
        ...
        if "/members?" in url:
            if not done("assert:search_results_loaded"):
                return assertion(
                    "search_results_loaded", "A selectable member result is displayed", "Select"
                )
            return act(
                "Open the first matching member record",
                "click",
                target_role="link",
                target_name="Select",
            )
        ...
        if url.rstrip("/").endswith(member_root):
            if capability == "meridian_core.update_member_information" and done(
                "click:save_changes"
            ):
                if not done("assert:member_information_updated"):
                    return assertion(
            ...
            if not done("assert:member_record_loaded"):
                return assertion(
                    "member_record_loaded",
                    "The requested MEMBER RECORD is displayed",
                    "MEMBER RECORD",
                )
            if capability == "meridian_core.update_member_information":
                return act(
                    "Open Update Member Information",
                    "click",
                    target_role="link",
                    target_name="Update Member Information",
                )
            if capability == "meridian_core.place_account_hold":
                return act(
                    "Open Place Account Hold",
                    "click",
                    target_role="link",
                    target_name="Place Account Hold",
                )
            return Planned(
                "finish", {"status": "give_up", "summary": "unsupported offline member workflow"}
            )
        ...
        if url.rstrip("/").endswith(f"{member_root}/update"):
            if not done("assert:update_form_loaded"):
                return assertion(
                    "update_form_loaded",
                    "The Update Member Information form is displayed",
            ...
            for field_name, label in (
                ...
                if current(field_name) != wanted:
                    return act(
                        f"Enter the supplied {label}",
                        "type",
                        text=wanted,
                        target_name_attr=field_name,
                    )
            return act(
                "Save the member information changes",
                "click",
                target_role="button",
                target_name="Save Changes",
            )
        ...
        if url.rstrip("/").endswith(f"{member_root}/hold"):
            if not done("assert:hold_form_loaded"):
                return assertion(
                    "hold_form_loaded",
                    "The Place Account Hold form is displayed",
                    "PLACE ACCOUNT HOLD",
                )
            if current("share") != str(params.get("share", "")):
                return act(
                    "Select the supplied share",
                    "select",
                    value=str(params.get("share", "")),
                    target_name_attr="share",
                )
            if current("reason") != str(params.get("reason_code", "")):
                return act(
                    "Select the supplied hold reason",
                    "select",
                    value=str(params.get("reason_code", "")),
                    target_name_attr="reason",
                )
            if current("notes") != str(params.get("notes", "")):
                return act(
                    "Enter the hold notes",
                    "type",
                    text=str(params.get("notes", "")),
                    target_name_attr="notes",
                )
            return act(
                "Continue to the hold review", "click", target_role="button", target_name="Continue"
            )
        ...
        if url.rstrip("/").endswith(f"{member_root}/hold/review"):
            if not done("assert:hold_review_loaded"):
                return assertion(
                    "hold_review_loaded",
                    "The hold review screen is displayed",
                    "CONFIRM ACCOUNT HOLD",
                )
            return act(
                "Post the account hold", "click", target_role="button", target_name="Place Hold"
            )
        ...
        if url.rstrip("/").endswith(f"{member_root}/hold/post"):
            ...
            if "confirmation_reference" in remaining:
                ...
                return act(
                    "Read the hold confirmation reference",
                    "read",
            ...
            if "hold_status" in remaining:
                status = next(
                    (
                        str(c.get("name"))
                        for c in controls
                        if str(c.get("name") or "").strip().upper() == "HOLD"
                    ),
                    "HOLD",
                )
                return act(
                    "Read the resulting hold status",
                    "read",
                    output="hold_status",
                    target_text=status,
                )
        ...
        return Planned(
            "finish", {"status": "give_up", "summary": f"unexpected MERIDIAN screen at {url}"}
        )
