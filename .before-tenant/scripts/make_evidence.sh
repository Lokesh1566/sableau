...
echo "== 1/5 discovery =="
"$PY" -m sableau.cli discover --job jobs/approve_claim.json --planner "$PLANNER" \
...
"$PY" -m sableau.cli validate --capability "$CAP" >> evidence/01_discovery.txt 2>&1

echo "== 2/5 deterministic replay with new parameters =="
curl -sf -X POST "$APP/admin/reset" >/dev/null
{
  echo "### replay 1: approve a different claim than the one discovery used"
  "$PY" -m sableau.cli replay --capability "$CAP" --confirm-risky \
...
curl -sf -X POST "$APP/admin/reset" >/dev/null
"$PY" scripts/demo_handoff.py > evidence/04_handoff.txt 2>&1

echo "== 5/5 tests =="
{
  echo "### unit suite, no browser required"
  "$PY" -m pytest tests/unit tests/test_no_llm_in_replay.py -v 2>&1 | tail -80
  echo
  echo "### integration suite, real Chromium against the live application"
  "$PY" -m pytest tests/integration -v 2>&1 | tail -25
} > evidence/05_tests.txt 2>&1

"$PY" scripts/index_evidence.py > evidence/README.md
echo
